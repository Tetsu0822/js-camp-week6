...
async function getProductsSafe() {
	...
	try {
		...
		if (!response.ok) {
			return { success: false, error: `HTTP 錯誤：${response.status}` };
		}
		const data = await response.json();
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：1xx：請求已收到，正在處理中。
	   2xx：請求成功完成。
	   3xx：重新導向，資源位置改變，需要額外動作。
	   4xx：用戶端錯誤，表示使用者送出的請求有問題。
	   5xx：伺服器端發生問題。
...
3. 什麼是 RESTful API？
   答：RESTful API 是一種使用 HTTP 規則來設計 API 的風格，把「資料」當成資源，並透過 HTTP Method 操作它。
